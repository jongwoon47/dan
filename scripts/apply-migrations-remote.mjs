...
await runSql(
  readFileSync("supabase/migrations/0008_prevent_duplicate_connections.sql", "utf8"),
  "0008",
);
await runSql(
  readFileSync("supabase/migrations/0009_security_p0_hardening.sql", "utf8"),
  "0009",
);
await runSql(
...
where schemaname = 'public'
  and indexname in ('matches_one_connected_response_uidx','matches_parties_demand_connected_uidx')
union all
select policyname from pg_policies
where schemaname = 'public'
  and policyname in (
    'responses_no_direct_insert','responses_no_direct_update',
    'demands_no_direct_update','activity_no_direct_update'
  )
union all
select tgname from pg_trigger where tgname = 'trg_sell_intent_ownership';
`.trim(),
  "VERIFY",
);
