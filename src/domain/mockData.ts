...
function buyFulfillment(index: number): FulfillmentOption[] {
  const patterns: FulfillmentOption[][] = [
    [{ mode: "SHIPPING" }, { mode: "MEETUP", place: placeFromLabel("성동구") }],
    [{ mode: "SHIPPING" }],
    [{ mode: "MEETUP", place: placeFromLabel("마포구 근처") }],
    [{ mode: "SHIPPING" }, { mode: "MEETUP", place: placeFromLabel("강남") }],
    [{ mode: "MEETUP", place: placeFromLabel("평택") }],
  ];
...
const SEED_BORROW: BorrowDemand[] = [
  {
    ...
    userId: "user-jun",
    type: "BORROW",
    title: "주말 빔프로젝터 빌려주실 분",
    description: "주말 발표에 쓸 빔프로젝터 구해요.",
    category: "rental",
    budget: 30000,
    fulfillmentOptions: [
      { mode: "PICKUP", place: placeFromLabel("성수동 근처") },
    ],
    status: "ACTIVE",
    createdAt: daysAgo(1),
...
const SEED_TASK: TaskDemand[] = [
  {
    ...
    userId: "user-hae",
    type: "TASK",
    title: "오늘 평택에서 케이크 받아주실 분",
    description: "케이크 가게에서 픽업해서 가져다주실 분 구해요.",
    category: "errand",
    budget: 15000,
    fulfillmentOptions: [
      { mode: "PICKUP", place: placeFromLabel("평택역 근처") },
    ],
    status: "ACTIVE",
    createdAt: daysAgo(0),
  ...
  },
  {
    ...
    userId: "user-mina",
    type: "TASK",
    title: "서류 받아서 가져다주실 분",
    description: "오늘 8시까지 서류 픽업 후 전달 부탁드려요.",
    category: "errand",
    budget: 20000,
    fulfillmentOptions: [
      {
    ...
    createdAt: daysAgo(0),
    expiresAt: daysFromNow(1),
    details: { taskDescription: "서류 픽업 후 전달", dueAt: daysFromNow(0) },
  },
  {
    id: "demand-task-remote-form",
    userId: "user-jun",
    type: "TASK",
    title: "온라인 신청서 작성 도와주실 분",
    description: "짧은 온라인 신청서 작성 도움 구해요.",
    category: "errand",
    budget: 10000,
    ...
    details: { taskDescription: "온라인 신청서 작성 도움" },
  },
];
...
const SEED_SERVICE: ServiceDemand[] = [
  {
    ...
    userId: "user-mina",
    type: "SERVICE",
    title: "이케아 가구 조립 부탁드려요",
    description: "책장 조립 도와주실 분 구해요.",
    category: "furniture",
    budget: 40000,
    fulfillmentOptions: [
      { mode: "ONSITE", place: placeFromLabel("성수동") },
    ],
    status: "ACTIVE",
    createdAt: daysAgo(1),
    expiresAt: daysFromNow(3),
    details: { serviceDescription: "이케아 책장 조립" },
  },
  {
    id: "demand-svc-ppt",
    userId: "user-hae",
    type: "SERVICE",
    title: "PPT 디자인 조금 수정해주세요",
    description: "기존 슬라이드 디자인만 살짝 다듬어 주세요.",
    category: "service",
    budget: 30000,
    fulfillmentOptions: [{ mode: "REMOTE" }],
    status: "ACTIVE",
    ...
    details: { serviceDescription: "PPT 디자인 수정" },
  },
];
